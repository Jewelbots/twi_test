#include "haptics_driver.h"
#include "Haptics.h"
#include "Actuator_Waveforms.h"
#include "pmic_driver.h"
#include "nrf_delay.h"
#include "SEGGER_RTT.h"

void haptics_init(void)
{
	Haptics_Init();
	Haptics_SetActuator(ACTUATOR_LRA);
	Haptics_RunAutoCal_LRA();
	uint8_t ret;
	ret = Haptics_Diagnostics(ACTUATOR_LRA);
	UNUSED_VARIABLE(ret);
}

unsigned char haptics_test_cal_diags(void)
{
	unsigned char ret;
	Haptics_RunAutoCal_LRA();

	ret = Haptics_Diagnostics(ACTUATOR_LRA);
	//ret = (ret & 0x8) >> 3;  //And to only get at one specific bit, comment out to see all bits for diagnosis
	return ret;
}

unsigned char haptics_test_run1(void)
{
//#ifdef PMIC
//	bool battery_is_charging = pmic_is_charging();
//	if (battery_is_charging) {
//		return 0;  //  Don't activate the motor if the battery  charging
//	}
//#endif
#if DRV2604
	Haptics_SendWaveform(Tick, ACTUATOR_LRA, TRIGGER_INTERNAL);
#endif
#if DRV2605
	// Send library waveform number to be played, 1 to 123, page 57 of manual
	// send 10 = double click
	Haptics_SendWaveIndex(10);
#endif
	return 0; //ret;
}

unsigned char haptics_test_run2(void)
{
//#ifdef PMIC
//	bool battery_is_charging = pmic_is_charging();
//	if (battery_is_charging) {

//		return 0;  //  Don't activate the motor if the battery charging
//	}
//#endif
#if DRV2604
	Haptics_SendWaveform(BuzzAlert, ACTUATOR_LRA, TRIGGER_INTERNAL);
#endif
#if DRV2605
	// Send library waveform number to be played, 1 to 123, page 57 of manual
	// send 47 = buzz
	Haptics_SendWaveIndex(47);
#endif
	
	return 0; //ret;
}

unsigned char haptics_test_run3(void)
{
	//unsigned char ret;
	//uint32_t err;
//#ifdef PMIC
//	bool battery_is_charging = pmic_is_charging();
//	if (battery_is_charging) {
//		return 0;  //  Don't activate the motor if the battery charging
//	}
//#endif
#if DRV2604
	Haptics_SendWaveform(BuzzAlert, ACTUATOR_LRA, TRIGGER_INTERNAL);
#endif
#if DRV2605
	// Send library waveform number to be played, 1 to 123, page 57 of manual
	// send 82 = transition up
	Haptics_SendWaveIndex(82);
#endif
	
	return 0; //ret;
}

unsigned char haptics_test_run4(void)
{
//#ifdef PMIC
//	//unsigned char ret;
//	//uint32_t err;
//	bool battery_is_charging = pmic_is_charging();
//	if (battery_is_charging) {
//		return 0;  //  Don't activate the motor if the battery charging
//	}
//#endif
#if DRV2604
	Haptics_SendWaveform(BuzzAlert, ACTUATOR_LRA, TRIGGER_INTERNAL);
#endif
#if DRV2605
	// Send library waveform number to be played, 1 to 123, page 57 of manual
	// send 119 = smooth hum
	Haptics_SendWaveIndex(119);
#endif
	
	return 0;
}

